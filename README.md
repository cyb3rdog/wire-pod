...
- An optional SDK app server (port `80`: bot remote-control features like eye color/volume/camera streaming, plus the jdocs pinger) that wire-pod uses as an SDK *client* to reach back into the robot -- architecturally separate from the core voice pipeline above, which never depends on it. Turn it off (and hide the dashboard's now-nonfunctional "Bot Settings" page) from the dashboard's **Server Settings → Advanced** page, or `SDK_ENABLED=false`/`WIREPOD_SDK_ENABLED` in Docker on first boot (see below). This is decoupled from whether port 80 itself is bound: `/ok` (and, if SDK is on, the SDK endpoints too) is registered on the same handler table `:8080` serves, so it stays reachable there regardless. Disable port 80 itself the same way (Advanced page, or `PORT80_ENABLED=false`) if you don't want a separate port 80 socket at all -- e.g. only forwarding `:8080` through a reverse proxy -- keeping in mind a paired robot's own connectivity check-in only ever asks for port 80 (`server_config.json`'s `"check"` field has no way to point it elsewhere) and won't get answered once this is off. Either toggle needs a container restart to actually rebind, whether changed on the Advanced page or in `compose.yaml`.
...
`compose.yaml` declares every setting it supports as an environment variable with a default, so you can override any of them via a `.env` file next to `compose.yaml` or `WIREPOD_FOO=bar docker compose up -d`, without editing the file itself. Every one of these has a dashboard equivalent (Server Settings → Weather/Knowledge Graph/STT Service/Advanced) and **only seeds the first boot that ever reads it** -- once wire-pod is running, change settings from the dashboard instead; it's authoritative from then on and survives restarts, and the env vars stop being consulted. Vosk's thermal management/grammar tuning, mDNS, the legacy `:8084` listener, the jdocs pinger, and the SDK app server/port-80 toggles live on the **Advanced** page specifically; the first four of those take effect immediately, the SDK/port ones need a container restart to actually rebind their listeners either way. See `compose.yaml` for the full list.
